...
def Train(directory, epochs, embedding_size, dropout,  lr, wd, random_seed):
    ...
    print('## edges:', g.number_of_edges()+g2.number_of_edges())         ########g.number_of_edges()=21720
...
##########model2########
        ...
        # g2_train = g2.edge_subgraph(train_eid2, preserve_nodes=True).to(device)
        ...
        # test_eid2 = g2.filter_edges(lambda edges: edges.data['test2']).to(device)
        test_eid2 = g2.filter_edges(lambda edges: edges.data['test2'])
        src_test2, dst_test2 = g2.find_edges(test_eid2)
        # rating_test2 = g2.edges[test_eid2].data['rating'].to(device)
        ...
        for epoch in range(epochs):
            ...
            for _ in range(10):
                    ...
                    loss_train = cross_entropy(score_train, rating_train).mean()  ##or sum()
                    ####################      ####################
                    # score_train2 = model2(g2_train, src_train2, dst_train2).to(device)
                    score_train2 = model2(g2_train, src_train2, dst_train2)
                    loss_train2 = cross_entropy2(score_train2, rating_train2).mean()  ##orsum()
                    # loss_train1=(loss_train2+loss_train)/2
                    loss_train1=loss_train2+loss_train
                    # loss_train1=0.1*loss_train2+0.9*loss_train
...
##################################
        model2.eval()
        with torch.no_grad():
            # h_test2 = model2.encoder(g2).to(device)
            # score_test2 = model2.decoder(h_test2[src_test2], h_test2[dst_test2]).to(device)
